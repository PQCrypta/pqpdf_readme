...
44 forensic engines. Sandbox execution. One-click sanitization. Zero retention. No account.
...
A valid `/ByteRange [o1 l1 o2 l2]` must start at offset 0, keep both segments within file bounds, and have its inner gap contain only the `/Contents` hex blob:

- `o1` ≠ 0 → `Signature: ByteRange Does Not Start at Offset 0` (High) — file header is unsigned
...
- **Feeds Correlation Engine** — named JS registry + OpenAction +70 bonus; DocMDP bypass + JS +100 (critical); DocMDP bypass + weak algorithm +105 (critical); DocMDP P=3 + JS +70; multiple DocMDP + sig +65; ByteRange non-zero start + MDP +90; all-zero /Contents +95 in Engine ㊹
